...
import { 
  ...
  FastForward, Rewind, Calendar, Clock,
  Monitor, Terminal, FileText, Globe, Keyboard,
  Volume2, VolumeX, Settings, Maximize2, Radio, Wifi
} from 'lucide-react';
...
function KeystrokeReplay() {
  ...
  useEffect(() => {
    if (isLiveMode) {
      ...
      socketRef.current.on('new_event', (event) => {
        ...
        setLiveEvents(prev => [...prev, event].slice(-100)); // Keep last 100 events
        ...
        // Process the keys to update display
        setLiveText(prev => {
          let text = prev;
          let i = 0;
            ...
            if (rawKeys[i] === '[') {
              ...
              if (end !== -1) {
                ...
                if (special === '[BACKSPACE]') {
                  text = text.slice(0, -1);
                } else if (special === '[CTRL+BACKSPACE]') {
                ...
                i = end + 1;
                if (special === '[ENTER]' && i < rawKeys.length && rawKeys[i] === '\n') {
                  i++;
                }
                continue;
              }
  ...
  const skipToEvent = (direction) => {
    ...
    if (direction === 'forward') {
      const nextEvent = timelineEvents.find((e, i) => 
        i > currentIdx && e.eventIndex !== timelineEvents[currentIdx]?.eventIndex
      );
      if (nextEvent) setCurrentTime(nextEvent.time);
    } else {
      const prevEvents = timelineEvents.filter((e, i) => 
        i < currentIdx - 1 && e.eventIndex !== timelineEvents[currentIdx - 1]?.eventIndex
      );
      if (prevEvents.length > 0) {
        const lastPrevEvent = prevEvents[prevEvents.length - 1];
        const eventStart = timelineEvents.find(e => e.eventIndex === lastPrevEvent.eventIndex);
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {computers.map(c => (
                <option key={c.id} value={c.id}>{c.computer_name}</option>
              ))}
            </select>
          </div>
              ...
              {isLiveMode ? (
                ...
                liveText || liveEvents.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-gray-500">
                    ...
                    <p className="text-xs mt-3 text-gray-600">Type something on a monitored device to see it here</p>
                  </div>
                )
              ) : (
                // Replay mode display
                timelineEvents.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-gray-500">
                    ...
                    <p className="text-sm mt-1">Select a time range and click "Load"</p>
                    <p className="text-xs mt-3 text-gray-600">Or click "Go Live" to see real-time keystrokes</p>
                  </div>
                )
              )}
            </div>
                  ...
                  return (
                    <div
                      ...
                      className={`absolute top-0 w-1 h-full ${
                        i === currentEventIndex ? 'bg-yellow-400' : 'bg-gray-600'
                      }`}
                      style={{ left: `${position}%` }}
                    />
                  );
                ...
                <div className="flex items-center gap-3 min-w-[100px] justify-end">
                  <button
                    ...
                    className={`p-2 rounded-lg transition-colors ${
                      soundEnabled ? 'text-orange-400 bg-orange-500/20' : 'text-gray-500 hover:text-gray-300'
                    }`}
                    title="Toggle Sound"
                  >
              ...
              <>
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-sm text-gray-300 truncate font-mono">
                        {event.data?.text || event.data?.keys?.substring(0, 30) || '...'}
                      </p>
                ...
                {liveEvents.length === 0 && (
                  <div className="text-center text-gray-500 py-8">
                    ...
                    <p className="text-sm text-green-400">Listening for events...</p>
                    <p className="text-xs mt-1 text-gray-600">Events will appear here in real-time</p>
                  </div>
                )}
              </>
            ) : (
