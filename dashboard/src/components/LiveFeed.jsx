...
import { 
  Keyboard, Monitor, Clipboard, FolderOpen, Cpu, Globe,
  Pause, Play, Filter, Download, Trash2
} from 'lucide-react';
import { format } from 'date-fns';

const EVENT_ICONS = {
  keystroke: { icon: Keyboard, color: 'text-orange-400', bg: 'bg-orange-500/20' },
  window_focus: { icon: Monitor, color: 'text-blue-400', bg: 'bg-blue-500/20' },
  clipboard_copy: { icon: Clipboard, color: 'text-purple-400', bg: 'bg-purple-500/20' },
  file_created: { icon: FolderOpen, color: 'text-green-400', bg: 'bg-green-500/20' },
  file_modified: { icon: FolderOpen, color: 'text-yellow-400', bg: 'bg-yellow-500/20' },
  file_deleted: { icon: FolderOpen, color: 'text-red-400', bg: 'bg-red-500/20' },
  file_moved: { icon: FolderOpen, color: 'text-cyan-400', bg: 'bg-cyan-500/20' },
...
function EventItem({ event }) {
  ...
  const getEventDescription = () => {
    ...
    switch (event.event_type) {
      ...
      case 'window_focus':
        return (
          <span>
            ...
            <span className="text-blue-300">{data.window_title?.substring(0, 60)}</span>
            {data.duration_seconds && (
              <span className="text-gray-500 ml-2">({data.duration_seconds}s)</span>
            )}
          </span>
        );
      case 'clipboard_copy':
        return (
          <span>
            Copied: <span className="text-purple-300 font-mono text-sm">
      ...
      case 'file_created':
        return <span>Created: <span className="text-green-300">{data.file_path}</span></span>;
      case 'file_modified':
        return <span>Modified: <span className="text-yellow-300">{data.file_path}</span></span>;
      case 'file_deleted':
        return <span>Deleted: <span className="text-red-300">{data.file_path}</span></span>;
      case 'file_moved':
        return (
          <span>
      ...
      case 'process_start':
        return <span>Started: <span className="text-pink-300">{data.process_name}</span></span>;
      case 'process_end':
        return (
          <span>
            Ended: <span className="text-gray-300">{data.process_name}</span>
            {data.duration_seconds && (
              <span className="text-gray-500"> ({Math.round(data.duration_seconds)}s)</span>
            )}
          </span>
        );
      default:
        return <span className="text-gray-400">{JSON.stringify(data).substring(0, 100)}</span>;
    }
  };
...
function LiveFeed({ events, selectedComputer }) {
  ...
  useEffect(() => {
    ...
    if (filter !== 'all') {
      filtered = filtered.filter(e => e.category === filter || e.event_type === filter);
    }
